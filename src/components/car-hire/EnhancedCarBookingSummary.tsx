...
interface EnhancedCarBookingSummaryProps {
  car: CarRental;
  location: string;
  pickupDate: Date;
  returnDate: Date;
...
export default function EnhancedCarBookingSummary({
  car,
  location,
  pickupDate,
  returnDate,
  ...
  userRole = 'user',
  discountCode
}: EnhancedCarBookingSummaryProps) {
  ...
  useEffect(() => {
    ...
    let price = car.pricePerDay * rentalDays;
    
    if (extras && extras.length > 0) {
      extras.forEach(extra => {
        price += extra.pricePerDay * extra.quantity * rentalDays;
      });
    }
  ...
  return (
    <div className="bg-white rounded-lg shadow-sm border border-gray-200 p-4 sm:p-6">
      ...
      {/* Pricing Breakdown with Service Integration */}
      {basePrice > 0 && (
        <PriceBreakdown
          basePrice={basePrice}
